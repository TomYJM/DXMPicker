...
-(id)initWithFrame:(CGRect)frame withPhotoUrl:(NSString *)photoUrl dictionary:(NSDictionary *)dic{
    self = [super initWithFrame:frame];
    if (self) {
        //添加图片
        if (photoUrl) {
//            [self.imageView sd_setImageWithURL:[NSURL URLWithString:photoUrl] placeholderImage:imageName(@"NewLoadBig") completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
//                self.imageView.frame=[self caculateOriginImageSizeWith:image];
//            }];
        }else {
            [self.imageView setImage:dic[@"Image"]];
            self.imageView.frame = [self caculateOriginImageSizeWith:dic[@"Image"]];
        }
    }
    return self;
}
